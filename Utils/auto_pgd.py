# Adapted from https://github.com/fra31/auto-attack/blob/master/autoattack/autopgd_base.py
...
class APGDAttack():
            ...
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    for i, model in enumerate(self.ens_surrogates):
                        ...
                        loss = loss_indiv.sum()
                        loss_list.append(self.weights[i]*loss)
    ...
    def perturb(self, ens_surrogates, weights, x, y=None, best_loss=False, x_init=None):
        """
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, target, surr, weights, y=None, x_init=None):
        """
        :param x:           clean images
        :param y:           clean labels, if None we use the predicted labels
        """
        self.y_target = target
        self.ens_surrogates = surr
        ...
        if self.verbose:
            ...
            print('target class {}'.format(target_class), '- restart {} - robust accuracy: {:.2%}'.format(counter, acc.float().mean()), '- cum. time: {:.1f} s'.format(time.time() - startt))
